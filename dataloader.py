...
def read_pocket(mol_path,
    ...
    #siteresidue_list = list(zip(atoms['atom_name'],atoms['subst_name']))
    siteatom_list = atoms['atom_name'].tolist()
    #print(f"siteresidue_list:{len(siteresidue_list),siteresidue_list}")

    if 'sasa' in features_to_use:
        #print(f"siteatom_list:{siteatom_list},siteresidue_list:{siteresidue_list},pop_path:{pop_path}")
        qsasa_data = extract_sasa_data(siteatom_list, siteresidue_list, pop_path)
        #print(f"qsasa_data:{len(qsasa_data),qsasa_data}")
...
def extract_sasa_data(siteatom_list,siteresidue_list, pop):
    ...
    with open(pop) as popsa:
        for line in popsa:
            ...
            if len(line_list) == 12:
                ...
                residue_type = line_list[2] + line_list[4]
                if residue_type in siteresidue_list and atom_name in siteatom_list:
                    qsasa = line_list[7]
                    atom_name_list.append(atom_name)
                    residue_list.append(residue_type)
                    qsasa_list.append(qsasa)
    #print(f"atom_name_list:{len(atom_name_list),atom_name_list}")
    #print(f"residue_list:{len(residue_list),residue_list}")
    ...
    #print(f"qsasa_new:{len(qsasa_new),qsasa_new}")
    # Matching amino acids from .mol2 and .out files and creating dictionary
    qsasa_data = []
    fullprotein_data = list(zip(atom_name_list,residue_list, qsasa_new))
    """
    ...
    """
    for i, (atom,residue) in enumerate(zip(siteatom_list,siteresidue_list)):
        #print(f"{i},{atom},{residue}")
        find_qsasa = False
        for j in range(len(fullprotein_data)):
            if fullprotein_data[j][0]==atom and fullprotein_data[j][1]==residue:
                qsasa_data.append(fullprotein_data[j][2])
                find_qsasa = True
                break
        if find_qsasa:
            continue
        #print(f"not found qsasa data for atom:{atom} of residue:{residue}, substituted by median")
        qsasa_data.append(median)
    #print(f"qsasa_data:{len(qsasa_data),qsasa_data}")
    return qsasa_data
